...
class TextDataTable(BaseDataTable):
    ...
    def __post_init__(self):
        ...
        self._SPRITE_LIST = RELEASE_SPRITES if self._release_or_kiosk == 'release' else KIOSK_SPRITES
        self._data = list()
    ...
    def _generate_text(self, fh: FileIO, sec3ct: int, data_start: int, block_start: int, offset: int, bit_shift: int, is_text: bool) -> list[Union['_Text', '_Sprite']]:
        ...
        for pos in range(sec3ct):
            ...
            if is_text:
                ...
                sprite = self._SPRITE_LIST.get(str(sprite_index), f"unk{hex(sprite_index)}")
                data_object = _Sprite(
                    position=sprite_pos,
                    data=sprite_data,
                    sprite=sprite
    ...
    def _generate_blocks(self, fh: FileIO, section_count: int, data_start: int) -> tuple[int, list['_TextLineFragment']]:
        ...
        for _ in range(section_count):
            ...
            ret_list.append(_TextLineFragment(
    ...
    def _generate_block_data(self, fh: FileIO) -> tuple[int, list['_TextLine']]:
        ...
        for _ in range(data_count):
            ...
            fh.seek(data_start)
            info = b"" if block_start < data_start else get_bytes(fh, block_start - data_start)
            
            ret_list.append(_TextLine(
                arr=info,
    ...
    def _parse_text(self, fh: FileIO, data_start: int, text_list: list[Union['_Text', '_Sprite']]):
    ...
    def _parse_blocks(self, fh: FileIO, data_start: int, block_list: list['_TextLineFragment']):
    ...
    def _parse_block_data(self, fh: FileIO, data_start: int, block_data_list: list['_TextLine']):
