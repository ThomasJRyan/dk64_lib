...
- Current baseline result: `66 passed, 485 subtests passed`.
...
6. After the binary and table layers are clearer, simplify F3DEX2 command parsing into a compact registry and command model. Done across `908f9b1`, `ea941e3`, and the shared command-helper cleanup.
   Commands parse through explicit byte reads, packed RSP/RDP fields have opcode-level coverage, and unknown opcodes now return `None`.

## Additional Fixes

- `Rom.get_cutscene_data()` now returns `CutsceneData` objects instead of `TextureData` objects. Done in `4afd225`.

## Current Test Baseline

- Latest verification command: `.venv/bin/python -m pytest -q`
- Latest result: `66 passed, 485 subtests passed`
