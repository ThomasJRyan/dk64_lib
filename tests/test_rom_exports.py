...
class RomExportTest(unittest.TestCase):
    ...
    def test_export_all_combines_supported_exports(self):
        rom = _fake_rom()
        rom.export_geometries = lambda folderpath, include_textures=True: [
            Path(folderpath) / f"textures_{include_textures}.obj"
        ]
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            exported = Rom.export_all(rom, tmpdir, include_textures=False)
            root = Path(tmpdir)

            self.assertEqual(
                exported,
                {
                    "geometries": [root / "geometries" / "textures_False.obj"],
